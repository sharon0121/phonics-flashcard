...
import {
  useBlockPuzzleWordSources,
  setBlockPuzzleWordSources,
  WORD_SOURCE_LABELS,
...
export default function BlockPuzzleSettingsView() {
  const wordSources = useBlockPuzzleWordSources();

  function toggleSource(key: WordSourceKey) {
    const next = wordSources.includes(key) ? wordSources.filter((k) => k !== key) : [...wordSources, key];
    setBlockPuzzleWordSources(next);
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)' }}>
      <div className="mx-auto w-full max-w-2xl flex-1 px-4 py-8">
        ...
        <h1 className="mt-2 text-2xl font-bold text-[var(--hero-gold)]">⚙️ 積木消除設定</h1>
        ...
        <div className="mt-6 rounded-xl border-2 border-[var(--hero-gold)] bg-white/5 p-4 text-zinc-200">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => setBlockPuzzleWordSources(ALL_WORD_SOURCES)}
