...
import {
  ...
  ALL_WORD_SOURCES,
  type WordSourceKey,
} from '@/lib/puyoSettings';

export default function PuyoSettingsView() {
  const wordSources = usePuyoWordSources();

  function toggleSource(key: WordSourceKey) {
    const next = wordSources.includes(key) ? wordSources.filter((k) => k !== key) : [...wordSources, key];
    setPuyoWordSources(next);
  }

  return (
    <div className="min-h-screen" style={{ backgroundColor: '#0b1130', color: '#e2e8f0' }}>
    <div className="mx-auto w-full max-w-2xl flex-1 px-4 py-8">
      ...
      <h1 className="mt-2 text-2xl font-bold" style={{ color: '#ffcc33' }}>
        ⚙️ 魔法氣泡設定
      </h1>

      <div className="mt-6 rounded-xl border-2 p-4" style={{ borderColor: '#ffcc33', background: 'rgba(255,255,255,0.05)' }}>
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold">📚 單字考題來源</h2>
          <button
            type="button"
            onClick={() => setPuyoWordSources(ALL_WORD_SOURCES)}
