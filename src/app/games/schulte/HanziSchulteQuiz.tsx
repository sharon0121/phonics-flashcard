...
function makeRound(remaining: HanziWord[], allWords: HanziWord[]): Round | null {
...
export default function HanziSchulteQuiz({ onBack }: Props) {
  ...
  const [remaining, setRemaining] = useState<HanziWord[]>([]);
  ...
  }, [stage, round, justSolved]);

  function startGame() {
    const fresh = shuffle(pool);
    setRemaining(fresh);
    setRound(makeRound(fresh, words));
    setWrongCount(0);
    setJustSolved(false);
    setSaved(false);
    ...
    if (choice.id === round.correct.id) {
      ...
      setTimeout(() => {
        if (nextRemaining.length === 0) {
          ...
          setRound(makeRound(nextRemaining, words));
          setJustSolved(false);
        }
      }, 700);
    } else {
  ...
  return (
    <div className="relative flex-1 rounded-xl border-2 border-[var(--hero-gold)] bg-gradient-to-br from-[#0a0118] via-[#12042a] to-[#01030f] p-4">
      <div className="flex w-full items-center justify-between">
        ...
        <span className="text-sm font-bold text-[var(--hero-gold)]">🈶 國字複習</span>
        <Link
      ...
      {pool.length < 2 ? (
        ...
        <div className="mt-8 flex flex-col items-center gap-4 text-center">
          <p className="text-sm text-zinc-300">
            聽發音點出正確的國字，全部 {pool.length} 個字都答對才算過關；答錯 {MAX_WRONG} 次或時間到就算失敗，請在 {timeLimitSec} 秒內完成！
          </p>
          <button
            type="button"
            onClick={startGame}
        ...
        <div className="mt-4 flex flex-col items-center gap-3">
          ...
          <button
            ...
            className="rounded-full bg-white/10 p-2 text-lg text-white hover:bg-white/20"
            aria-label="再聽一次"
          >
            🔊 再聽一次
          </button>
